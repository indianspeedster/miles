...
`pr-test-rocm.yml` is a `workflow_dispatch`-only mirror of the GPU path for AMD MI300X runners: its `resolve-ci-image` (image family `rocm/sgl-dev:<tag>`) feeds one stage, `stage-c-8-gpu-mi300x`, which calls `_run-ci-rocm.yml` — the ROCm counterpart of `_run-ci.yml` (ROCm device flags, `rocm-smi` GPU-ready wait, no CPU job). It runs the `stage-c-8-gpu-mi300x` suite unsharded, under HIP/CUDA compatibility, and is not part of PR gating. Selection is explicit rather than inherited from the CUDA roster: a test runs on MI300X only if it carries a `register_rocm_ci(suite="stage-c-8-gpu-mi300x")` call, so enabling or disabling one there is a one-line change in the test file, independent of its CUDA registration. Because the MI300X runner is 8-GPU-only, tests registered on the narrower `stage-c-4-gpu-*` / `stage-c-2-gpu-*` CUDA suites are registered here too when they are wanted on ROCm, and occupy the full node. SGLang / Megatron-LM versions come baked into the ROCm image (`skip_dependency_install` defaults to `true` in `_run-ci-rocm.yml`), so the dispatch has no dependency-override inputs.
